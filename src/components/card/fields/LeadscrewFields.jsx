import { NumberInput } from "../../ui";

export function LeadscrewFields({ comp, onUpdate, theme }) {
  return (
    <>
      <div className="fr">
        <span className="fl" style={{ color: theme.sub }}>
          Lead
        </span>
        <NumberInput value={comp.lead} min={0.001} step={0.1} onChange={(v) => onUpdate("lead", v)} theme={theme} />
        <span className="fu" style={{ color: theme.sub }}>
          {comp.lsUnit}/rev
        </span>
      </div>
      <div className="fr">
        <span className="fl" style={{ color: theme.sub }}>
          Units
        </span>
        <div style={{ display: "flex", gap: 4 }}>
          {["in", "mm"].map((unit) => (
            <button
              key={unit}
              className={`ct ${comp.lsUnit === unit ? "on" : ""}`}
              style={{ padding: "4px 12px", fontSize: 10, color: theme.text }}
              onClick={() => onUpdate("lsUnit", unit)}
            >
              {unit}
            </button>
          ))}
        </div>
      </div>
      <div className="fr">
        <span className="fl" style={{ color: theme.sub }}>
          Efficiency
        </span>
        <NumberInput value={comp.efficiency} min={0} max={100} step={0.5} onChange={(v) => onUpdate("efficiency", v)} theme={theme} />
        <span className="fu" style={{ color: theme.sub }}>
          %
        </span>
      </div>
    </>
  );
}
